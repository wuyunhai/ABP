...
namespace SecondABP.Tests
{
    public abstract class SecondABPTestBase : AbpIntegratedTestBase<SecondABPTestModule>
    {
        ...
        protected void UsingDbContext(Action<SecondABPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<SecondABPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<SecondABPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<SecondABPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<SecondABPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SecondABPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<SecondABPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<SecondABPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<SecondABPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<SecondABPDbContext, Task<T>> func)
        {
            T result;
